...
def scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=0.0,
            is_causal=False, scale=None, enable_gqa=False, need_weights=False) -> torch.Tensor:
...
def apply_rotary_pos_emb(q, k, pos, position_ids=None, unsqueeze_dim=1, block_end_index: Optional[torch.Tensor] = None):
    ...
    if query_len == pos[0].shape[1]:
        q_embed = (q * pos[0].unsqueeze(unsqueeze_dim)) + (rotate_half(q) * pos[1].unsqueeze(unsqueeze_dim))
        k_embed = (k * pos[0].unsqueeze(unsqueeze_dim)) + (rotate_half(k) * pos[1].unsqueeze(unsqueeze_dim))
    else:
        q_embed = (q * pos[2].unsqueeze(unsqueeze_dim)) + (rotate_half(q) * pos[3].unsqueeze(unsqueeze_dim))
        k_embed = (k * pos[2].unsqueeze(unsqueeze_dim)) + (rotate_half(k) * pos[3].unsqueeze(unsqueeze_dim))
...
class DreamAttention(nn.Module):
    ...
    def forward(
        ...
        lengths: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,
        output_attentions: bool = False,
        use_cache: bool = False,
        rotary_pos: Optional[Tuple[torch.Tensor, torch.Tensor]] = None, 
        block_idx = None,
    ) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:

        query_position, full_position, track_position, query_masked_position, masked_position = positions
        ...
        bsz, q_len, _ = normed_hidden_states.size()
        ...
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, rotary_pos)
        ...
        att_weight = torch.matmul(query_states, key_states.transpose(2, 3)) / math.sqrt(self.head_dim)
        
        # upcast attention to fp32
        att_weight = nn.functional.softmax(att_weight, dim=-1, dtype=torch.float32).to(query_states.dtype)
        att_weight = nn.functional.dropout(att_weight, p=self.attention_dropout, training=self.training)
        attn_output = torch.matmul(att_weight, value_states)
        ...
        if block_idx >= start_reset:
            masked_att_weight = att_weight[:, :, query_masked_position, :]
        else:
            masked_att_weight = att_weight[:, :, -query_masked_position.shape[0]:, :]
            att_weight = att_weight[:, :, :track_position.shape[0], :]
            scale_factor = 1 / math.sqrt(past_q.size(-1))
...
class DreamDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        attention_mask: Optional[torch.Tensor] = None,
        positions: Optional[torch.LongTensor] = None,
        lengths: Optional[Tuple[torch.Tensor]] = None,
        output_attentions: Optional[bool] = False,
        use_cache: Optional[bool] = False,
        rotary_pos: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,  # will become mandatory in v4.46
        block_idx = None,
        **kwargs,
        ...
        # Self Attention
        hidden_states, attn_output = self.self_attn(
...
class DreamBaseModel(DreamPreTrainedModel):
    ...
    def forward(
        ...
        attention_mask: Optional[torch.Tensor] = None,
        positions: Optional[torch.LongTensor] = None,
        lengths: Optional[List[torch.FloatTensor]] = None,
        ...
        pos = self.rotary_emb(hidden_states, positions[1].unsqueeze(0))
        rotary_pos = [pos[0], pos[1]]     

        pos = self.rotary_emb(hidden_states, positions[0].unsqueeze(0))
        rotary_pos += [pos[0], pos[1]]        
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            # if layer_idx == (len(self.layers) - 1): past_key_values[1] = layer_idx
