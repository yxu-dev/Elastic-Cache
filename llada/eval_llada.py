...
# Increase or decrease the cache size limit
# dynamo.config.cache_size_limit = 256  # default is 64
# You can also tweak the accumulated cache limit
# dynamo.config.accumulated_cache_size_limit = 512  # default 64
...
class LLaDAEvalHarness(LM):
    def __init__(
        ...
        **kwargs,
        ...
        self.block_caching = block_caching
    ...
    def generate_until(self, requests):
        ...
        for i, req in enumerate(tqdm(requests, desc="Generating...")):
            ...
            generated_answer, nfe, ratio = generate_with_elastic_cache(
                self.model, input_ids, gen_length=self.gen_length, window_length=self.window_length, 
                mask_id=self.mask_id, eos_id=126081,
                threshold=self.threshold, tokens_per_iter=self.tokens_per_iter,
                gamma=self.gamma, track_num=self.track_num, block_caching=self.block_caching,
            )
            num_samples += 1
            ratios += ratio
        ...
        if self.show_speed:
            ...
            print(f"Avg Cache update frequency:: {ratios / num_samples}")
        return output
