...
class CustomRotaryEmbedding(RotaryEmbedding):
    """
    [Custom Rotary positional embeddings].
    """

    def __init__(self, config: ModelConfig, cache: BufferCache):
        super().__init__(config, cache)
    
    def forward(self, q: torch.Tensor, k: torch.Tensor, pos) -> Tuple[torch.Tensor, torch.Tensor]:
        if self.config.rope_full_precision:
            q_, k_ = q.float(), k.float()
        else:
            q_, k_ = q, k
        ...
        with torch.autocast(q.device.type, enabled=False):
            if q_.shape[2] == pos[0].shape[2]:
                q_ = self.apply_rotary_pos_emb(pos[0], pos[1], q_)
                k_ = self.apply_rotary_pos_emb(pos[0], pos[1], k_)
            else:
                q_ = self.apply_rotary_pos_emb(pos[2], pos[3], q_)
                k_ = self.apply_rotary_pos_emb(pos[2], pos[3], k_)
...
class LLaDABlock(nn.Module):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        if self.config.rope:
            ...
            self.rotary_emb = CustomRotaryEmbedding(config, self.__cache)
    ...
    def attention(
        ...
        if block_idx >= start_reset: # Cache update
            ...
            self.k_cache[:, :, query_position, :] = k
            self.v_cache[:, :, query_position, :] = v

            past_q = self.q_cache[:, :, track_position, :].clone()
            self.q_cache[:, :, query_position, :] = q
        ...
        att, att_weight = self._scaled_dot_product_attention(
            q,
            k,
            v,
            attn_mask=None,
...
class CustomLMOutputWithPast(NamedTuple):
    ...
    att_weights: Optional[List[torch.Tensor]]
    """
...
class LLaDAModel(nn.Module):
    ...
    def forward(
        ...
        if not (self.config.alibi or self.config.rope):
            ...
            pos_emb = self.transformer.wpe(positions[0].unsqueeze(0))  # type: ignore
            x = pos_emb + x
        ...
        if (
            ...
            if attention_bias is None and self.config.alibi:
                attention_bias = get_causal_attention_bias(
                    self.__cache, x.shape[1], x.device
                ) + self.get_alibi_attention_bias(x.shape[1], x.device)
            elif attention_bias is None:
                attention_bias = get_causal_attention_bias(self.__cache, x.shape[1], x.device)
        ...
        key_len = lengths[0] 
        pos_sin, pos_cos = self.transformer.blocks[0].rotary_emb.get_rotary_embedding(key_len, x.device)
        if self.config.rope_full_precision:
            pos_sin, pos_cos = pos_sin.float(), pos_cos.float()
        rotary_pos = [pos_sin[:, :, positions[0], :], pos_cos[:, :, positions[0], :], pos_sin, pos_cos]
        ...
        return CustomLMOutputWithPast(logits=logits, att_weights=None, hidden_states=tuple(all_hidden_states) if output_hidden_states else None)
